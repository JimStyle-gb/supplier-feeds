...
"""
Build Akcent YML (flat <offers>) for Satu
script_version = akcent-2025-10-02.5

Что нового в .5:
- FEED_META оставлен в формате из feed.txt, «Ближайшее время сборки (Алматы)» = 02:00.
- Порядок тегов внутри <offer>: <vendorCode>, <name>, <price>, <picture>, <vendor>, <currencyId>, <available>, <description>.
- <offer id="…"> синхронизирован с <vendorCode> (id = vendorCode).
- Нормализация бренда:
  • <vendor>ViewSonic proj</vendor> → <vendor>ViewSonic</vendor>
  • <vendor>Epson Proj</vendor>     → <vendor>Epson</vendor>
  • Если <vendor>Китай</vendor>, пробуем угадать бренд по <name>/<description> (ViewSonic, Epson, BenQ, Optoma, Acer, Xiaomi, JMGO, Anker,
    Nebula, LG, Samsung, Sony, Panasonic, Philips, Huawei). Если нашли — подставляем.
"""
...
# ==================== КЛЮЧИ: СТАРТОВЫЕ ФРАЗЫ/РЕГУЛЯРКИ ====================
...
# ——— правила замены для Akcent ———
VENDOR_REPLACEMENTS = {
    _norm_key("ViewSonic proj"): "ViewSonic",
    _norm_key("Epson Proj")   : "Epson",
}
# Список брендов, если vendor == "Китай" — пробуем угадать из name/description.
BRAND_GUESS_LIST = [
...
def normalize_brand(raw: str) -> str:
    k=_norm_key(raw)
    if not k: return ""
    # Точные замены типа "ViewSonic proj" → "ViewSonic"
    if k in VENDOR_REPLACEMENTS:
        return VENDOR_REPLACEMENTS[k]
    # Блокируем имена поставщиков и «не бренды»
...
def ensure_vendor(shop_el: ET.Element) -> Tuple[int, Dict[str,int]]:
    """
    Нормализуем <vendor>:
    - "ViewSonic proj" → "ViewSonic"; "Epson Proj" → "Epson"
    - Если "Китай": ищем бренд в <name>/<description>; при нахождении подставляем
    - Маркеры «unknown/no brand/…» — удаляем тег
    """
    ...
    for offer in offers_el.findall("offer"):
        ...
        # Китай → угадываем бренд по name/description
        if _norm_key(txt) == _norm_key("Китай"):
            ...
            if m:
                ...
                # не нашли — оставим как есть (Китай), но можно удалить при желании:
                # offer.remove(ven)
                pass
            continue

        # Обычная нормализация + замены (ViewSonic proj/Epson Proj)
        canon = normalize_brand(txt)
        if canon and canon != txt:
            ven.text = canon; normalized += 1
        elif not canon:
            # canon пустой — удалить тег
            if ven is not None: offer.remove(ven)
            key=_norm_key(txt); 
            if key: dropped[key]=dropped.get(key,0)+1
...
def main()->None:
    ...
    # Нормализация брендов (с особыми правилами), vendorCode
    ...
    # Параметры → описание; затем однострочное описание; затем удаляем <param>
    ...
    # Сериализация и косметика: OFFSEP → пустые строки; разрыв после FEED_META
...
def _norm_name(s: str) -> str:
    """Нормализация: NBSP -> пробел, ё->е, нижний регистр, схлоп пробелов."""
    s = (s or "").replace("\u00A0"," ").lower().replace("ё","е")
    return re.sub(r"\s+"," ",s).strip()
...
class KeySpec:
    """kind='prefix' (нормализованная фраза-префикс) или 'regex' (паттерн; матчим с начала через .match)."""
    __slots__=("raw","kind","norm","pattern")
    def __init__(self, raw: str, kind: str, norm: Optional[str], pattern: Optional[re.Pattern]):
        self.raw, self.kind, self.norm, self.pattern = raw, kind, norm, pattern
...
# Не считаем брендами имена поставщиков
SUPPLIER_BLOCKLIST={_norm_key(x) for x in["alstyle","al-style","copyline","akcent","ak-cent","vtt"]}
UNKNOWN_VENDOR_MARKERS=("неизвест","unknown","без бренда","no brand","noname","no-name","n/a")
...
def main()->None:
    ...
    # Нормализация брендов, vendorCode
    norm_cnt, dropped_names = ensure_vendor(out_shop)
    total_prefixed, created_nodes, filled_from_art, _ = ensure_vendorcode_with_article(
        out_shop, prefix=os.getenv("VENDORCODE_PREFIX","AC"),
        create_if_missing=os.getenv("VENDORCODE_CREATE_IF_MISSING","1").lower() in {"1","true","yes"}
