...
def load_keywords(path: str) -> List[KeySpec]:
    ...
    for ln in data.splitlines():
        ...
        if s.startswith("~="):
            w=_norm_name(s[2:]); 
            if not w: continue
            keys.append(KeySpec(s,"word",re.compile(r"\b"+re.escape(w)+r"\b",re.I))); continue
...
def name_matches(name:str,keys:List[KeySpec])->Tuple[bool,Optional[str]]:
    n=_norm_name(name)
    for ks in keys:
        if ks.kind=="substr" and ks.raw and ks.raw in n: return True,ks.raw
        if ks.kind!="substr" and ks.pattern and ks.pattern.search(name or ""): return True,ks.raw
    return False,None
