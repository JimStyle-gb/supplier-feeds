...
Изменения:
- Фильтр по ключевым словам теперь применяется ПОСЛЕ ensure_vendor(), чтобы видеть нормализованный <vendor>.
- <url> сохраняем до фильтра, после — удаляем в финальной чистке.
- FEED_META.offers_written считает уже после фильтра; добавлен filtered_by_keywords.

Прочее (как было):
- Файл ключевых слов: docs/akcent_keywords.txt (режим include/exclude).
- Удаляем <categoryId>, служебные теги/атрибуты; переносим характеристики в description (без SPECS_*).
- Авто-наценка, хвост ...900, currency=KZT.
- vendorCode: если пуст/префикс — берём артикул (offer@article -> из <name> -> из <url> -> offer@id), затем префикс "AC".
"""
...
PURGE_TAGS_AFTER = (
    ...
    "url"  # удаляем <url> ПЕРЕД записью итогового файла
)
...
def offer_text_blob(offer: ET.Element) -> str:
    ...
    add(offer.findtext("url"))  # для отбора (в финале удалим)
...
def normalize_brand(raw: str) -> str:
    ...
    if k in ALLOWED_BRANDS_CANON_MAP: return ALLOWED_BRANDS_CANON_MAP[k]
    if k in _BRAND_MAP:
...
def extract_brand_any(offer: ET.Element) -> str:
    ...
    return extract_brand_from_params(offer) or scan_text_for_allowed_brand(name) or scan_text_for_allowed_brand(desc)
...
def ensure_vendor(shop_el: ET.Element) -> Tuple[int,int,int,Dict[str,int]]:
    ...
    for offer in offers_el.findall("offer"):
        ...
        if candp:
            ET.SubElement(offer, "vendor").text = candp; filled_param += 1; continue

        candt = extract_brand_any(offer)
        if candt:
            ET.SubElement(offer, "vendor").text = candt; filled_text += 1; continue
...
def main() -> None:
    ...
    if shop_in is None and src_root.tag.lower() == "shop": shop_in = src_root
    ...
    # Базовый выходной документ
    out_root = ET.Element("yml_catalog"); out_root.set("date", time.strftime("%Y-%m-%d %H:%M"))
    out_shop = ET.SubElement(out_root, "shop")
    out_offers = ET.SubElement(out_shop, "offers")

    # Копируем офферы, удаляем categoryId (без фильтра ЗДЕСЬ)
    for o in src_offers:
        mod = deepcopy(o)
    ...
    # === 1) Сначала нормализуем/восстанавливаем vendor ===
    norm_cnt, fill_param_cnt, fill_text_cnt, dropped_names = ensure_vendor(out_shop)

    # === 2) Теперь фильтр по ключевым словам (vendor уже есть) ===
    keywords = load_keywords(AKCENT_KEYWORDS_PATH)
    use_filter = AKCENT_KEYWORDS_MODE in {"include","exclude"} and len(keywords) > 0
    filtered_out = 0
    if use_filter:
        for off in list(out_offers.findall("offer")):  # список, чтобы безопасно удалять
            blob = offer_text_blob(off)
            hit = match_keywords(blob, keywords)
            if (AKCENT_KEYWORDS_MODE == "exclude" and hit) or (AKCENT_KEYWORDS_MODE == "include" and not hit):
    ...
    # Финальная чистка мусора (в т.ч. <url> и offer-атрибутов)
