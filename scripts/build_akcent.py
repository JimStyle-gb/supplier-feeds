...
# Делает: Грубая транслитерация RU->латиница + слаг для keywords.
...
# Делает: Собрать строку keywords из вендора, названия и городов РК.
...
@dataclass
# Делает: описывает структуру OfferData.
class OfferData:
    id: str
    available: str
    category_id: str
...
# Делает: Попробовать угадать кодировку.
...
# Делает: Немного привести бренд к виду для vendor/Производитель.
...
# Делает: Применить наценку 4% + фиксированный диапазон и хвост 900.
...
# Делает: Достать пары (name, value) из Param и список сопутствующих устройств.
...
# Делает: Собрать HTML <description> на основе родного описания поставщика.
def _build_description(name: str, raw_desc: str, params: list[tuple[str, str]], compat: list[str]) -> str:
    ...
    html_block = "\n".join(inner)
    return f"\n{html_block}\n"
# Делает: Попробовать угадать бренд по Param/имени/описанию.
...
# Делает: Разобрать один исходный <offer> в структуру OfferData или вернуть None, если выкидываем.
...
# Делает: Скачать исходный XML от поставщика.
...
# Делает: Экранировать текст для тела XML-тега, не трогая кавычки.
def _escape_text(text: str) -> str:
    return html.escape(text or "", quote=False)

# Делает: Собрать финальный YML как строку.
def _build_yml(offers: list[OfferData], total_raw: int) -> str:
    ...
    for off in offers:
        ...
        html_desc = off.description_html.replace("]]>", "]]]]><![CDATA[>").strip("\n")
        lines.append(html_desc)
...
# Делает: Главная точка входа: скачать, пересобрать, сохранить.
