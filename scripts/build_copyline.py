...
"""
Build Copyline YML feed from XLSX + site pages (photos, full descriptions, site categories).
More robust XLSX reader: auto-detects header row, scans all sheets, flexible column aliases.
"""
...
# =========================
NAME_ALIASES = [
    "наименование товара", "наименование", "название", "товар", "продукт",
    "номенклатура", "позиция", "описание", "модель", "item", "product", "name"
]
PRICE_ALIASES = [
    "цена", "цена, тг", "цена тг", "цена (тг)", "цена, тг., с ндс", "цена с ндс",
    "розничная цена", "стоимость", "price", "цена kzt", "цена в тенге"
]
SKU_ALIASES = [
    "артикул", "код", "код товара", "vendorcode", "sku", "part", "партномер", "pn"
]
...
def _coerce_price_series(sr: pd.Series) -> pd.Series:
    # keep as float if already numeric
    if pd.api.types.is_numeric_dtype(sr):
        return pd.to_numeric(sr, errors="coerce")
    # normalize string prices: remove currency, spaces, use dot as decimal
    s = sr.astype(str).str.strip()
    s = s.str.replace(r"[^\d,.\- ]", "", regex=True)
    s = s.str.replace(" ", "", regex=False)
    # if both comma and dot exist, assume comma thousands -> remove commas
...
def _pick_column_by_alias(df: pd.DataFrame, aliases: List[str]) -> Optional[str]:
    low_map = { _normalize_header_cell(c): c for c in df.columns }
    for a in aliases:
        a_norm = _normalize_header_cell(a)
        for c_low, c_real in low_map.items():
            if c_low == a_norm or c_low.startswith(a_norm):
                return c_real
    return None

def _guess_price_column(df: pd.DataFrame) -> Optional[str]:
    best_col, best_score = None, -1.0
    for c in df.columns:
        serie = _coerce_price_series(df[c])
        nonnull = serie.notna().sum()
        if nonnull == 0:
            continue
        # price-like if most values positive and reasonable magnitude
        pos = (serie > 0).sum()
        score = pos / max(1, len(serie))
        if score > best_score:
            best_score = score
            best_col = c
    return best_col
...
def _guess_name_column(df: pd.DataFrame) -> Optional[str]:
    # Prefer string-heavy column with longer average length
    best_col, best_len = None, -1.0
    for c in df.columns:
        s = df[c].astype(str)
        # exclude columns that look numeric mostly
        nonnum_ratio = (s.str.contains(r"[A-Za-zА-Яа-я]", regex=True)).mean()
        avg_len = s.str.len().mean()
        score = nonnum_ratio * avg_len
        if score > best_len:
            best_len = score
            best_col = c
    return best_col

def _reheader_if_needed(raw_df: pd.DataFrame) -> pd.DataFrame:
    """If headers are not on first row, try to detect header row by alias hit."""
    df = raw_df.copy()
    # Quick check: if any alias matches current headers, keep as is
    if any(_pick_column_by_alias(df, NAME_ALIASES + PRICE_ALIASES)):
        return df

    # Try scan first 10 rows to find a row that contains aliases
    scan_rows = min(10, len(df))
    for i in range(scan_rows):
        row_vals = [_normalize_header_cell(v) for v in df.iloc[i].tolist()]
        row_str = " | ".join(row_vals)
        if any(any(a in rv for rv in row_vals) for a in NAME_ALIASES + PRICE_ALIASES):
            # Re-read with this row as header
            new = df.iloc[i+1:].copy()
            new.columns = [str(x).strip() for x in df.iloc[i].tolist()]
            return new
    return df  # fallback
...
def read_xlsx(url: str) -> pd.DataFrame:
    ...
    xfile = pd.ExcelFile(io.BytesIO(b), engine="openpyxl")
    ...
    for sheet in xfile.sheet_names:
        try:
            raw = xfile.parse(sheet_name=sheet, header=0, dtype=object)
        except Exception:
            raw = xfile.parse(sheet_name=sheet, header=None, dtype=object)
        if raw is None or raw.empty:
            continue

        df = _reheader_if_needed(raw)
        ...
        # Try find columns
        name_col = _pick_column_by_alias(df, NAME_ALIASES) or _guess_name_column(df)
        price_col = _pick_column_by_alias(df, PRICE_ALIASES) or _guess_price_column(df)
        sku_col = _pick_column_by_alias(df, SKU_ALIASES)

        if not name_col or not price_col:
            # keep looking on other sheets
            continue

        # Build normalized frame
        tmp = pd.DataFrame({
            "name": df[name_col].astype(str),
            "price": _coerce_price_series(df[price_col]),
        })
        if sku_col:
            tmp["sku"] = df[sku_col].astype(str)
        else:
            tmp["sku"] = ""
        ...
        tmp = tmp[~tmp["name"].eq("")]
        tmp = tmp[tmp["price"].fillna(0) > 0]
        if tmp.empty:
            continue

        # Pick the sheet with most valid rows
        if best_df is None or len(tmp) > len(best_df):
            best_df = tmp.reset_index(drop=True)
            chosen_sheet = sheet

    if best_df is None:
        raise RuntimeError("Нет обязательных столбцов 'Название'/'Цена' (не удалось найти на листах).")

    print(f"[xls] sheet: {chosen_sheet}, rows: {len(best_df)}, cols={list(best_df.columns)}")
    return best_df
...
def build_startswith_patterns(keywords: List[str]) -> List[re.Pattern]:
    ...
    for kw in keywords:
        ...
        if not kw:
            continue
        kw_esc = re.escape(kw)
        # начинаем с ключевого слова, но допускаем бренд перед ним
...
# Main building logic
# =========================
def fetch_site_pages_for_tokens(tokens_norm: set) -> Dict[str, Dict]:
    ...
    def url_ok(u: str) -> bool:
        ...
        if any(t in path_norm for t in tokens_norm):
            return True
        for hint in ["drum","dr-","dr","drunit","drum-unit",
                     "toner","cartridge","tn-",
                     "developer","dev-",
...
def main():
    ...
    # 2) Keywords (must start with keyword; allow brand prefix like "RIPO ")
    ...
    if fdf.empty:
        raise RuntimeError("После фильтрации по ключам не осталось ни одного товара из XLSX.")

    # 3) Prepare tokens from filtered rows
    all_tokens = set()
    ...
    # 4) Crawl site pages for those tokens (to grab photo/desc/breadcrumbs)
    page_data = fetch_site_pages_for_tokens(norm_tokens)
    if not page_data:
        raise RuntimeError("Не удалось разобрать ни одной карточки сайта (нет кандидатов).")

    # 5) Map tokens -> urls for faster matching
    ...
    # 6) Category tree builder from breadcrumbs
    ...
    def pick_url_for_row(row: pd.Series) -> Optional[str]:
        ...
        for t in tokens:
            ...
            for u in token2urls.get(nt, []):
                candidates.append(u)
        if not candidates:
            # fallback by keyword overlap in product h1
    ...
    # 8) Assemble offers (keep only items whose SITE H1 also starts with our keywords rule and has picture)
    offers: List[Dict] = []
    ...
    # 9) Emit categories in parent->child order
    ...
    def emit_chain(name: str):
        ...
        if parent and parent != ROOT_CAT_NAME:
            emit_chain(parent)
            pid = name_parent2id[(parent, parent_of.get(parent))]
        else:
            pid = ROOT_CAT_ID
    ...
    for name in list(parent_of.keys()):
        emit_chain(name)
