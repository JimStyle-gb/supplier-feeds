...
# кэш для картинок (метод 2: поиск «на лету»)
IMG_CACHE_FILE       = os.getenv("IMG_CACHE_FILE", "docs/copyline_photos.json")
IMG_LOOKUPS_PER_RUN  = int(os.getenv("IMG_LOOKUPS_PER_RUN", "300"))  # ограничиваем за прогон
IMG_WORKERS          = int(os.getenv("IMG_WORKERS", "6"))            # параллельность
...
def fetch_xlsx(url_or_path: str) -> bytes:
    if re.match(r"^https?://", url_or_path, re.I):
        r = requests.get(url_or_path, headers=UA_HEADERS, timeout=REQ_TIMEOUT)
        r.raise_for_status()
        return r.content
...
def save_img_cache(cache: Dict[str, Any]):
    d = os.path.dirname(IMG_CACHE_FILE)
...
def _fetch(url: str) -> Optional[str]:
    try:
        r = requests.get(url, headers=UA_HEADERS, timeout=REQ_TIMEOUT)
...
def _first_main_img_src(soup: BeautifulSoup) -> Optional[str]:
    # БЕРЁМ ТОЛЬКО ПЕРВЫЙ <img itemprop="image"> → src
    img = soup.find("img", {"itemprop":"image"})
    if img and img.get("src"):
        return absolutize(img.get("src"))
    # fallback на jshopping путь
