...
script_version = copyline-2025-09-20.1

Изменения:
- URL фото берём ТОЛЬКО из img[itemprop="image"][id^="main_image_"] → @src.
  Никаких ог-фолбэков: если <img> нет — <picture> не записываем.
- Бренд и описание остаются как прежде: бренд из brand/.manufacturer_name,
  описание+ТХ из div[itemprop="description"].jshop_prod_description.
"""
...
RETRY_BACK  = float(os.getenv("RETRY_BACKOFF_S", "2.0"))
...
def find_header(ws, scan_rows: int = 80, max_cols: int = 40):
...
def name_starts_with(name: str, keys: List[str]) -> bool:
...
def scrape_product_details(url: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    ВАЖНО: picture берём ТОЛЬКО из img[itemprop="image"][id^="main_image_"] → @src.
    """
    html = fetch_html(url)
    if not html: return None, None, None
    soup = BeautifulSoup(html, "html.parser")

    # 1) Фото: строго из <img itemprop="image" id="main_image_..."> @src
    picture = None
    ...
    # 3) Описание + ТХ: из div.jshop_prod_description
    desc = None
...
def render_feed_meta(pairs: Dict[str, str]) -> str:
    ...
    comments = {
        ...
        "picture_found":"Сколько товаров получили фото (main_image_…)",
...
def main():
    ...
    if KEYWORDS_MODE == "include" and not keys:
        die("COPYLINE_KEYWORDS_MODE=include, но список ключей пуст.")
    ...
    meta = {
        ...
        "rows_after_cat_filter": str(rows_after_cat),
        "rows_after_keyword_filter": str(rows_after_keys),
