...
def fetch_xlsx(url_or_path: str) -> bytes:
    if re.match(r"^https?://", url_or_path, re.I):
        r = requests.get(url_or_path, headers=UA_HEADERS, timeout=60); r.raise_for_status()
        return r.content
...
def best_header(ws):
    def score(arr):
        low=[norm(x).lower() for x in arr]; got=set()
        for k,hints in HEADER_HINTS.items():
            for cell in low:
                if any(h in cell for h in hints): got.add(k); break
        return len(got)
    rows=[]
...
def build_yml(items):
    ...
    for it in items:
        ...
        for tag in ("quantity_in_stock","stock_quantity","quantity"):
            SubElement(o,tag).text="1"  # всегда 1
...
def main():
    ...
    for ws in wb.worksheets:
        ...
        if cols.get("name") is None and cols.get("article") is None:
            continue
        ...
        for row in ws.iter_rows(min_row=start, values_only=True):
            ...
            # пропускаем строки-заголовки (нет артикула и нет цены)
            if not (name or article): continue
            if (not article) and (price is None): continue
        ...
        break  # берём первый подходящий лист
