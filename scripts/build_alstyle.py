...
ПОВЕДЕНИЕ (выполняется по порядку):
  ...
  5) Чистка параметров: удаляем <param> с именами из списка пользователя (Артикул, Благотворительность, Код ТН ВЭД,
     ...
     пробелов/знаков препинания; поддержана форма «Штрих-код».

Выход: docs/alstyle.yml в кодировке Windows-1251 (cp1251) с корректной XML-декларацией.
Все изменения делаются безопасно (удаление по копии, аккуратная нормализация).
"""
...
# Добавлены разумные варианты написания («Штрих-код» наряду с «Штрихкод»).
PARAMS_TO_DROP = {
    "артикул",
    "благотворительность",
    "код тн вэд",
