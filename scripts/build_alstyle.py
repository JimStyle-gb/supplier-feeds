...
ВЕРСИЯ: alstyle-2025-10-30.v7.4.0
ИЗМЕНЕНО:
- Оформление «родного описания» перенесено в самый конец пайплайна (фаза финальной сборки XML).
- В <description> сначала ставим плейсхолдер [[[NATIVE]]]{сырой HTML}[[[/NATIVE]]].
- На последнем шаге _replace_html_placeholders_with_cdata() вызывает format_native_description_html()
  и подставляет уже отформатированный блок вместо плейсхолдера.
- Исправлены огрехи рендеринга (дубли заголовков, разорванные пункты, повтор «Размеры и вес», и т.п.).
"""
...
SCRIPT_VERSION = "alstyle-2025-10-30.v7.4.0"
...
def compose_full_description_html(lead_html: str, native_placeholder_block: str, specs_html: str, faq_html: str, reviews_html: str) -> str:
    pieces=[]
    if lead_html: pieces.append(lead_html)
    if native_placeholder_block: pieces.append(native_placeholder_block)  # здесь плейсхолдер [[[NATIVE]]]{raw}[[[/NATIVE]]]
...
def inject_seo_descriptions(shop_el: ET.Element) -> Tuple[int, str]:
    """
    ВНИМАНИЕ: тут НЕ форматируем «родное описание».
    Сохраняем его как есть внутрь плейсхолдера [[[NATIVE]]]{RAW}[[[/NATIVE]]].
    Финальное оформление — в _replace_html_placeholders_with_cdata() (самый конец пайплайна).
    """
    offers_el=shop_el.find("offers")
    if offers_el is None: return 0, ""
    ...
    for offer in offers_el.findall("offer"):
        ...
        native_placeholder = f"[[[NATIVE]]]{raw_desc_html_full}[[[/NATIVE]]]"
        full_html = compose_full_description_html(lead_html, native_placeholder, specs_html, faq_html, reviews_html)
...
# ======================= ОФОРМЛЕНИЕ РОДНОГО ОПИСАНИЯ (КОНЕЧНАЯ СТАДИЯ) =======================
BLOCK_TAG_SPLIT_RE = re.compile(r"</(p|li|h\d)>", re.I)

def _clean_text_lines_from_html(raw_html: str) -> List[str]:
    if not raw_html: return []
    txt = raw_html
    txt = re.sub(r"(?i)<br\s*/?>", "\n", txt)
    txt = BLOCK_TAG_SPLIT_RE.sub(r"\n", txt)
    txt = re.sub(r"<[^>]+>", " ", txt)
    txt = _unescape(txt)
    txt = strip_noise_chars(txt)
    txt = re.sub(r"\s{2,}", " ", txt)
    lines = [normalize_free_text_punct(ln).strip(" -–—•\t") for ln in txt.split("\n")]
    return [ln for ln in lines if ln]

def _merge_broken_port_lines(lines: List[str]) -> List[str]:
    """Склеивает разорванные строки типа '2×' на одной и 'Thunderbolt 5...' на следующей."""
    out=[]
    i=0
    while i < len(lines):
        cur = lines[i]
        if i+1 < len(lines):
            nxt = lines[i+1]
            if re.fullmatch(r"\d+\s*[×x]\s*", cur, re.I) or re.fullmatch(r"\d+\s*[×x]\b.*", cur, re.I):
                # пример: '2×' + 'Thunderbolt 5 (DP, PD)'
                cur = (cur + " " + nxt).strip()
                i += 1
        out.append(cur)
        i += 1
...
def _sectionize(lines: List[str]) -> Dict[str, List[str]]:
    sec = {"Ключевые характеристики": [], "Порты и подключения": [], "Автономность и питание": [],
           "Безопасность и мультимедиа": [], "Размеры и вес": []}
    for ln in lines:
        low = ln.lower()
        if any(k in low for k in ["процессор","видеокарт","экран","частота","матриц","озу","накопител","охлажден","клавиатур","корпус"]):
            sec["Ключевые характеристики"].append(ln); continue
        if any(k in low for k in ["thunderbolt","usb","hdmi","sd","rj-45","lan","аудиоразъём","аудиоразъем","wi-fi","bluetooth","порт","подключен"]):
            sec["Порты и подключения"].append(ln); continue
        if any(k in low for k in ["батаре","аккум","заряд","ос:","free dos","windows","linux","autonomy","питани"]):
            sec["Автономность и питание"].append(ln); continue
        if any(k in low for k in ["камера","динамик","nahimic","hi-res","tpm","kensington","безопасност","мультимед"]):
            sec["Безопасность и мультимедиа"].append(ln); continue
        if any(k in low for k in ["габарит","размер","вес","цвет","cosmos gray"]):
            sec["Размеры и вес"].append(ln); continue
        # если ничего не подошло — кидаем в «Ключевые характеристики», чтобы не потерять смысл
        sec["Ключевые характеристики"].append(ln)
    # Убираем дубли
...
def _render_sections_to_html(sec: Dict[str,List[str]]) -> str:
    parts=[]
    order=["Ключевые характеристики","Порты и подключения","Автономность и питание","Безопасность и мультимедиа","Размеры и вес"]
    for head in order:
        items = sec.get(head, [])
        if not items: continue
        parts.append(f"<h3>{_html_escape_in_cdata_safe(head)}</h3>")
        parts.append("<ul>")
        for it in items:
            parts.append(f"  <li>{_html_escape_in_cdata_safe(it)}</li>")
        parts.append("</ul>")
    return "\n".join(parts)

def format_native_description_html(raw_html: str) -> str:
    """
    Делает из сырого HTML аккуратные секции:
    - Убирает мусор, дубли заголовков («Размеры и вес» повторно и т.п.).
    - Склеивает разрывы строк (2× / Thunderbolt и т.п.).
    - Возвращает готовый HTML (без CDATA).
    """
    lines = _clean_text_lines_from_html(raw_html)
    if not lines: return ""
    lines = _merge_broken_port_lines(lines)
    # Убираем возможные дубли «Размеры и вес» / лишние «и подключения»
    lines = [re.sub(r"^\s*и подключения\s*$","Порты и подключения", ln, flags=re.I) for ln in lines]
    # Переупорядочиваем в секции
    sec = _sectionize(lines)
    html = _render_sections_to_html(sec)
    return html
...
# ======================= ФИНАЛЬНАЯ ЗАМЕНА ПЛЕЙСХОЛДЕРОВ (ПОСЛЕДНИЙ ШАГ) =======================
def _replace_html_placeholders_with_cdata(xml_text: str) -> str:
    # 1) Превращаем [[[HTML]]]...[[[/HTML]]] → CDATA <description>...</description>
    def repl(m):
        inner = m.group(1)
        inner = inner.replace("[[[HTML]]]", "").replace("[[[/HTML]]]", "")
        # 2) Обрабатываем «родное описание» на ЭТОМ ШАГЕ
        def repl_native(mm):
            raw = mm.group(1)
            pretty = format_native_description_html(raw)
            return pretty
        inner = re.sub(r"\[\[\[NATIVE\]\]\](.*?)\[\[\[\/NATIVE\]\]\]", repl_native, inner, flags=re.S)
        inner = _unescape(inner)
        inner = _html_escape_in_cdata_safe(inner)
        return f"<description><![CDATA[\n{inner}\n]]></description>"

    xml_text = re.sub(r"<description>(\s*\[\[\[HTML\]\]\].*?\[\[\[\/HTML\]\]\]\s*)</description>", repl, xml_text, flags=re.S)
    return xml_text
...
def main()->None:
    ...
    # CATEGORY ID → 0 первым (удаляем исходные, проставим позже)
    ...
    # ВСТАВЛЯЕМ SEO-блоки С ПЛЕЙСХОЛДЕРОМ «РОДНОГО ОПИСАНИЯ»
    ...
    # Аккуратный перенос после комментария FEED_META
    xml_text=re.sub(r"(-->)\s*(<shop\b)", r"\1\n\2", xml_text, count=1)
    # Разделяем офферы пустой строкой
    xml_text=re.sub(r"(</offer>)\s*\n\s*(<offer\b)", r"\1\n\n\2", xml_text)
    # Сжимаем лишние пустые строки
    xml_text=re.sub(r"(\n[ \t]*){3,}", "\n\n", xml_text)

    # САМЫЙ ПОСЛЕДНИЙ ШАГ: подстановка CDATA и оформление родного описания
