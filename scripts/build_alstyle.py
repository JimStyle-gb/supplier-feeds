...
# -*- coding: utf-8 -*-
"""
AlStyle → YML конвертер (CDATA-safe, quotes-on)
Обновлено: 2025-10-20

Изменения:
- Внутри CDATA кавычки " не экранируются (вариант №2).
- По-прежнему экранируем &, <, > и безопасно разрезаем ']]>'.
- Остальная логика сохранена как в предыдущей версии.
"""
...
SCRIPT_VERSION = "alstyle-2025-10-20.cdata-safe-quotes-on"
...
# ----------- ВАЖНО: экранирование без кавычек (для CDATA) -----------
def esc_noq(s: str) -> str:
    """Экранирует только &, <, > (кавычки не трогаем)."""
    if s is None: return ""
...
def render_html_description(product_name: str,
    ...
    parts: List[str] = []
    if product_name:
        parts.append("<h3>" + esc_noq(product_name.strip()) + "</h3>")
    if cleaned_desc:
        paragraphs = [p.strip() for p in re.split(r"\n\s*\n", cleaned_desc) if p.strip()]
        for p in paragraphs:
            p_html = esc_noq(p).replace("\n", "<br>")
            parts.append("<p>" + p_html + "</p>")
    if features:
        parts.append("<h3>Особенности</h3>")
        parts.append("<ul>")
        for ftxt in features:
            parts.append("  <li>" + esc_noq(ftxt) + "</li>")
        parts.append("</ul>")
    if specs_pairs:
        parts.append("<h3>Характеристики</h3>")
        parts.append("<ul>")
        for name, val in specs_pairs:
            parts.append("  <li><strong>" + esc_noq(str(name).strip()) + ":</strong> " + esc_noq(str(val).strip()) + "</li>")
        parts.append("</ul>")
    return "\n".join(parts).strip()
...
def _to_cdata_safe(s: str) -> str:
    # Разрываем любые ']]>' внутри, затем оборачиваем
    s = (s or "").replace("]]>", "]]]]><![CDATA[>")
    return "<![CDATA[\n" + s + "\n]]>"

def _postprocess_descriptions_to_cdata(xml_text: str) -> str:
    # 1) [[HTML]]…[[/HTML]] -> CDATA (безопасно)
    def _desc_html_to_cdata(m: re.Match) -> str:
        inner = m.group(1).strip()
        # НЕ разворачиваем HTML сущности здесь, чтобы не превратить &amp; в & и не ломать разметку
    ...
    xml_text = re.sub(r"</description>\s*</description>", "</description>", xml_text)
