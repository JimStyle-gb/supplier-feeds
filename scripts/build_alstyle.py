...
def _ensure_footer_spacing(text: str) -> str:
    # existing spacing preserved; add final footer spacing and compact descriptions
    text = re.sub(r'</offer>\s*(?=</offers>)', '</offer>\n\n', text, flags=re.I|re.S)
    text = re.sub(r'</offers>\s*(?=</shop>)', '</offers>\n', text, flags=re.I|re.S)
    text = re.sub(r'</shop>\s*(?=</yml_catalog>)', '</shop>\n', text, flags=re.I|re.S)
    text = re.sub(r'(<shop>\s*<offers>)', r'\1\n\n', text, flags=re.I|re.S)
    def _pack(m):
        head, html, tail = m.group(1), m.group(2), m.group(3)
        html = re.sub(r'>\s+<', '><', html)
        html = re.sub(r'\s*(</?(?:p|h3|ul|li|div|hr)\b[^>]*>)\s*', r'\1', html)
        html = re.sub(r'[ \t]{2,}', ' ', html)
        html = re.sub(r'\n+', ' ', html)
        html = re.sub(r'\s+(</)', r'\1', html)
        return head + html + tail
    text = re.sub(r'(?is)(<\s*description\b[^>]*>)(.*?)(</\s*description\s*>)', _pack, text)
    return text
