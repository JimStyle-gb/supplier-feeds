...
Ключевое:# -*- coding: utf-8 -*-
"""
Универсальный генератор YML для al-style (единый шаблон).

Ключевое:
- STRICT_VENDOR_ALLOWLIST по умолчанию ВЫКЛЮЧЕН (блокируем только имена поставщиков).
- Цены: берём минимальную «дилерскую», применяем правила (4% + надбавка по диапазонам),
  затем округляем до вида …900.
- Характеристики: переносим в описание [SPECS_BEGIN]…[SPECS_END], «Вес» сохраняем и нормализуем.
- Удаляем служебные цены и нежелательные <param> (Артикул/Штрихкод/Код ТН ВЭД/Код и всё, что не относится к товару:
  акции/скидки/«снижена цена», благотворительность, статусы, доставка/самовывоз, рассрочка/кредит, НДС/налоги, ед. измерения и т.п.).
- FEED_META — многострочный и понятный (key = value  | пояснение).
"""
...
VENDORCODE_CREATE_IF_MISSING = os.getenv("VENDORCODE_CREATE_IF_MISSING", "1").lower() in {"1","true","yes"}  # создаём по умолчанию

# Политика по брендам: по умолчанию СТРОГИЙ режим ВЫКЛЮЧЕН
...
# Удаляем эти параметры всегда (до сборки характеристик) — НЕ товарные/служебные
UNWANTED_PARAM_KEYS = {
    # твои обязательные
    "артикул","штрихкод","код тн вэд","код",
    # промо/маркетинг/статусы
    "снижена цена","скидка","акция","уценка","новинка","хит продаж","топ продаж","лидер продаж","лучшая цена","рекомендуем","подарок","кэшбэк","кешбэк","предзаказ","статус",
    # логистика/продажи/оформление
    "доставка","самовывоз","срок поставки","наличие","в наличии","резерв","кредит","рассрочка",
    # учет/налоги/ед. измерения/сертификация/служебные
    "единица измерения","базовая единица","vat","ндс","налог","сертификат","сертификация",
    # лишние из примеров поставщиков
    "благотворительность",
}

# После встраивания характеристик — удаляем все <param>, кроме явно разрешённых (перечислить через env)
...
# ===================== HTTP GET c If-Modified-Since/ETag =====================
...
def scan_text_for_allowed_brand(text: str) -> str:
    if not text:
        return ""
    for rg, val in _BRAND_PATTERNS:
        if rg.search(text) and brand_allowed(val):
            return val
    for rg in [
...
def extract_brand_from_name(name: str) -> str:
    if not name:
        return ""
    for rg, val in _BRAND_PATTERNS:
        if rg.search(name) and brand_allowed(val):
            return val
    for rg in [
...
# скрыть «служебные» пункты из характеристики по имени (регэксп по имени параметра)
...
def build_specs_lines(offer: ET.Element) -> List[str]:
    ...
    for p in offer.findall("param"):
        ...
        k = _key(raw_name)
        # 1) выключаем всё служебное/не товарное
        if k in UNWANTED_PARAM_KEYS:
            continue
        if EXCLUDE_NAME_RE.search(raw_name):
            continue

        # 2) нормализация ожидаемых товарных значений
        ...
        # 3) «мусорные» значения вроде кодов
        if (not is_weight) and _looks_like_code_value(raw_val):
            continue

        # 4) явные неинформативные булевы поля по общим названиям
...
- Цены: берём минимальную «дилерскую», применяем правила (4% + надбавка по диапазонам),
  затем округляем до вида …900.
- Характеристики: переносим в описание [SPECS_BEGIN]…[SPECS_END], «Вес» сохраняем и нормализуем.
- Удаляем служебные цены и нежелательные <param> (Артикул/Штрихкод/Код ТН ВЭД/Код).
...
# Канонические бренды (сокращённая видимая часть; в твоём репо держим полный список 753 бренда)
ALLOWED_BRANDS_CANONICAL = [
    "HP","Canon","Brother","Kyocera","Xerox","Ricoh","Epson","Samsung","Panasonic",
    "Konica Minolta","Sharp","Lexmark","Pantum",
    "APC","Eaton","Legrand","Dahua","CyberPower","Europrint","ViewSonic","HyperX","Mr.Pixel","NV Print",
...
def brand_allowed(canon: str) -> bool:
    # В мягком режиме разрешён любой бренд (кроме поставщиков)
...
def normalize_brand(raw: str) -> str:
    ...
    for rg, val in _BRAND_PATTERNS:
        if rg.search(raw or ""):
            return val if brand_allowed(val) else ""
    # 4) мягкий режим — автокапитализация
    if not STRICT_VENDOR_ALLOWLIST:
        return " ".join(w.capitalize() for w in k.split())
    # 5) строгий режим — ничего
    return ""

def scan_text_for_allowed_brand(text: str) -> str:
    if not text: return ""
    # быстрые паттерны известных брендов
    for rg, val in _BRAND_PATTERNS:
...
def extract_brand_from_name(name: str) -> str:
    ...
    # префиксы в скобках/квадратных и т. п.
    for rg in [
        re.compile(r"^\s*\[([^\]]]{2,30})\]\s+", re.U),
        re.compile(r"^\s*\(([^\)]{2,30})\)\s+", re.U),
        re.compile(r"^\s*([A-Za-zА-ЯЁЇІЄҐ][A-Za-z0-9А-ЯЁЇІЄҐ\-\.\s]{1,20})\s+[-–—]\s+", re.U),
