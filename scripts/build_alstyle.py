...
- Удаление &quot; / &#34; внутри CDATA-описаний (оставляем обычные ").
...
SCRIPT_VERSION = "alstyle-2025-10-20.cdata-safe-h3-name.fix-quot"
...
STOPWORDS_RU = {
    ...
    "бумага","бумаги","бумаг","черный","чёрный","белый","серый","цвет","оригинальный",
    "комплект","набор","тип","модель","модели","формат","новый","новинка"
}
STOPWORDS_EN = {
    "for","and","or","with","of","the","a","an","to","in","on","by","at","from",
...
def main()->None:
    ...
    # >>> ДОБАВЛЕНО: убрать &quot; / &#34; ТОЛЬКО внутри CDATA блоков описания
    def _cdata_fix_quotes(m: re.Match) -> str:
        inner = m.group(1)
        inner = inner.replace("&quot;", '"').replace("&#34;", '"')
        return "<![CDATA[" + inner + "]]>"

    xml_text = re.sub(r"<!\[CDATA\[(.*?)\]\]>", _cdata_fix_quotes, xml_text, flags=re.S)
    # <<< КОНЕЦ ДОБАВЛЕНИЯ
