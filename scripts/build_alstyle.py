...
version: alstyle-2025-10-16.strict-05a
delta:
- remove only:
  • <param name="Код товара Kaspi">…</param>
  • "Назначение: Да" (в <param> и в блоке «Характеристики» в <description>)
...
SCRIPT_VERSION = "alstyle-2025-10-16.strict-05a"
...
# --------------------- categories helpers (same as before) ---------------------
class CatRule:
...
# --------------------- brands/pricing/availability (unchanged) ---------------------
...
# --- описание/kv парсинг (укорочено: всё как в твоей текущей версии) ---
...
def normalize_kv(name: str, value: str) -> Tuple[str, str]:
    ...
    v = re.sub(r"^\s*(?::\s*)+", "", v)  # убираем ведущие «:»
    n_l = n.lower()
...
def unify_specs_in_description(shop_el: ET.Element) -> Tuple[int,int]:
    ...
    for offer in offers_el.findall("offer"):
        ...
        # --- ФИЛЬТР РОВНО «Назначение: Да» из описаний (не трогаем другие значения)
        kv_from_desc = [
        ...
        # страховка: выкинуть из финального набора «Назначение: Да» (если каким-то образом пролезло)
        merged = {
...
# --------------------- availability/currency/order/keywords/feed_meta (как было) ---------------------
...
# --- keywords / feed_meta как было (опущено ради краткости, не менял) ---
...
def main()->None:
    ...
    if ALSTYLE_CATEGORIES_MODE in {"include","exclude"}:
        ...
        if ALSTYLE_CATEGORIES_MODE=="include" and not (rules_ids or rules_names):
            err("ALSTYLE_CATEGORIES_MODE=include: нет правил категорий", 2)
    ...
    # === ТОЛЬКО ТО, ЧТО ПРОСИЛ: удаляем «Код товара Kaspi» и «Назначение: Да»
    remove_specific_params(out_shop)

    # сбор и унификация характеристик (с фильтром «Назначение: Да» внутри)
    ...
    # валюта
    for _ in [0]: pass  # (если у тебя уже была фикса валюты — оставь как в прежней версии)
    # порядок + categoryId первым
    # (если в твоей базе ты уже это делаешь — оставь)
