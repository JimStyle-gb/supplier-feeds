...
- Ценообразование по твоей таблице PRICING_RULES + округление к ближайшим 10
"""
...
UA = {"User-Agent": "Mozilla/5.0 (compatible; VTT-Feed/2.3)"}
...
def parse_price_kzt(soup: BeautifulSoup) -> Optional[int]:
    ...
    if el:
        ...
        if val is not None: return int(round(val))
...
def first_image_url(soup: BeautifulSoup) -> Optional[str]:
    ...
    for tag in soup.find_all(True):
        ...
        if isinstance(style, str) and "background-image" in style:
            ...
            if m and (".jpg" in m.group(1).lower() or ".png" in m.group(1).lower()): return abs_url(m.group(1))
        for attr in ("src","data-src","href","data-img","content"):
            v = tag.get(attr)
            if not v or not isinstance(v, str): continue
            vl = v.lower()
            if "/images/" in vl and (vl.endswith(".jpg") or vl.endswith(".png")): return abs_url(v)
    return None
...
def extract_description_meta(soup: BeautifulSoup) -> str:
    ...
    if tag and tag.get("content"): return re.sub(r"\s+", " ", tag["content"].strip())
...
def parse_product(s: requests.Session, url: str) -> Optional[Dict[str,Any]]:
    ...
    soup = soup_of(b)
    title = (soup.select_one(".page_title") or soup.title or soup.find("h1"))
...
# ---------------- FEED_META + YML ----------------
def almaty_now_str() -> str: return (datetime.utcnow() + timedelta(hours=5)).strftime("%Y-%m-%d %H:%M:%S +05")
...
def main() -> int:
    ...
    # parse + price
    parsed = 0; priced_count = 0; offers: List[Dict[str,Any]] = []; seen_ids: Set[str] = set()
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        for fut in as_completed([ex.submit(worker, u) for u in prod_urls]):
            ...
            if not it: continue
            parsed += 1
            src = it["price_src"]; new_price = apply_pricing(src)
            if new_price != src: priced_count += 1
            it["price"] = new_price
            oid = it["id"]
            if oid in seen_ids:
    ...
    print(f"[done] items: {len(offers)} -> {OUT_FILE}"); return 0
