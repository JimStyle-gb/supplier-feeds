...
def _enrich_sparse_device_desc(bits: list[str], pmap: dict[str, str]) -> list[str]:
    """
    Если у техники мало сильных техпараметров, добираем безопасные опорные поля.
    Это не мусор и помогает raw быть полезнее.
    """
    strong_payload_count = max(0, len(bits) - 1)  # первый элемент обычно сам тип
...
def _desc_for_computer(pmap: dict[str, str]) -> str:
    ...
    cpu = _join_nonempty(
        [pmap.get("Серия процессора", ""), pmap.get("Модель процессора", "")],
        sep=" ",
    )
    ...
    storage = _join_nonempty(
        [pmap.get("Объем жесткого диска", ""), pmap.get("Тип жесткого диска", "")],
        sep=" ",
    )
    ...
    os_name = _join_nonempty(
        [pmap.get("Операционная система", ""), pmap.get("Версия операционной системы", "")],
        sep=" ",
    )
    _append_param_line(bits, "ОС", os_name)

    gpu = _join_nonempty(
        [pmap.get("Марка чипсета видеокарты", ""), pmap.get("Модель чипсета видеокарты", "")],
        sep=" ",
    )
