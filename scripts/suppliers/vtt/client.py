...
VTT probe client.
v5:
- логин по legacy source.py flow;
- allow_paths теперь реально ограничивает обход;
- deny_patterns расширены, чтобы не лезть в аккаунт/новости/документы;
- это всё ещё временный probe-клиент.
"""
...
class VTTClient:
    ...
    def _looks_authenticated(self, html: str, current_url: str) -> bool:
        ...
        if "/catalog" in path and "вход для клиентов" not in text:
            return True
        protected_markers = ["каталог", "корзина", "заказы", "личный кабинет", "выйти", "избранное"]
        return any(marker in text for marker in protected_markers) and "вход для клиентов" not in text
        ...
        while queue and len(results) < max_pages:
            ...
            try:
                ...
                if "text/html" not in ctype:
                    results.append(
                        {"url": resp.url, "status_code": resp.status_code, "content_type": ctype, "kind": "non_html"}
                    )
                    continue
                ...
                for link in links:
                    ...
                    if parsed_link.scheme not in {"http", "https"}:
                        continue
                    if allow_paths and not any(parsed_link.path.startswith(p) for p in allow_paths):
                        continue
    ...
    def classify_page(url: str, html: str) -> str:
        ...
        if "/catalog" in lower_url and ("в корзину" in lower_html or "add to cart" in lower_html):
            return "product_like"
        if "/catalog" in lower_url and ("товаров" in lower_html or "показать" in lower_html):
            return "listing_like"
