...
def _extract_compat(title: str, vendor: str, params: Sequence[tuple[str, str]], desc: str) -> str:
    for key, value in params:
        ...
        if any(x in k for x in ("совмест", "для устройств", "для принтеров", "подходит")):
            ...
            if val:
                return val
    title_clean = _norm_ws(title)
    m = re.search(r"\bдля\s+(.+?)(?:(?:,\s*(?:\d+(?:[.,]\d+)?\s*[kк]|black|cyan|magenta|yellow|grey|gray|red|blue|photo ?black|matt?e?black|ч[её]рн|ж[её]лт|син|голуб|малинов|пурпур|сер|крас))|$)", title_clean, re.I)
    if not m:
        return ""
...
def _norm_color(value: str) -> str:
    ...
    mapping = {
        "black": "Черный", "photo black": "Черный", "photoblack": "Черный", "matte black": "Черный",
        "matt black": "Черный", "matteblack": "Черный", "mattblack": "Черный", "черный": "Черный",
        "чёрный": "Черный", "bk": "Черный", "cyan": "Голубой", "синий": "Голубой", "голубой": "Голубой",
        "c": "Голубой", "yellow": "Желтый", "желтый": "Желтый", "жёлтый": "Желтый", "y": "Желтый",
        "magenta": "Пурпурный", "малиновый": "Пурпурный", "пурпурный": "Пурпурный", "m": "Пурпурный",
        "grey": "Серый", "gray": "Серый", "серый": "Серый", "red": "Красный", "красный": "Красный",
    }
...
def _infer_color_from_title(title: str) -> str:
    ...
    checks = [
        ("photo black", "Черный"), ("photoblack", "Черный"), ("matte black", "Черный"), ("matt black", "Черный"),
        ("matteblack", "Черный"), ("mattblack", "Черный"), (" black ", "Черный"), ("чёрный", "Черный"),
        ("черный", "Черный"), (" bk", "Черный"), (" cyan", "Голубой"), ("синий", "Голубой"),
        ("голубой", "Голубой"), (" c,", "Голубой"), (" yellow", "Желтый"), ("жёлтый", "Желтый"),
        ("желтый", "Желтый"), (" y,", "Желтый"), (" magenta", "Пурпурный"), ("малиновый", "Пурпурный"),
        ("пурпурный", "Пурпурный"), (" m,", "Пурпурный"), (" grey", "Серый"), (" gray", "Серый"),
        ("серый", "Серый"), (" red", "Красный"), ("красный", "Красный"),
    ]
...
def build_offer_from_raw(raw: dict, *, id_prefix: str = "VT") -> OfferOut | None:
    ...
    for k, v in params:
        ...
        if k == "Цвет" and not color:
            color = _norm_color(v)
    desc = _build_native_desc(title, type_name, model, compat, resource, color, _s(raw.get("description_body") or raw.get("description_meta")))
