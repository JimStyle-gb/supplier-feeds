...
script_version = nvprint-2025-09-24.3a

Делает:
- id = <Артикул> с обрезкой только ведущего "NV-" (пример: "NV-CF232A-SET2" -> "CF232A-SET2")
- vendorCode = "NP" + id (пример: "NPCF232A-SET2")
...
def build_from_offers_tree(src_root: ET.Element, kw_pats: List[re.Pattern]) -> ET.Element:
    ...
    # Собираем все <offer> (без учета namespace/регистра)
    offers_in=[]
    ...
    for off in offers_in:
        ...
        if not article:
            # если у поставщика нет <Артикул> внутри offer — пропускаем (по правилу)
            continue

        offer_id = normalize_id_from_article(article)
        vcode    = make_vendor_code(offer_id)

        # Копируем разрешенные дочерние теги, КРОМЕ categoryId/quantity/available (добавим свой available)
