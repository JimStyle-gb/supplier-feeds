...
Что поправлено:
- Блок вида:#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
price_seo.py — Этап 2.4
-----------------------------------------------
Фикс: внутри блока
  <p>Характеристики</p>
  <p>Модель 001R00613</p>
  <p>Совместимые модели Xerox AltaLink C8030/...</p>
  <p>Ресурс 160 000 страниц (А4)</p>
преобразуем строки «Ключ Значение» в <li><strong>Ключ:</strong> Значение</li>
даже когда между ними только ОДИН пробел (без двоеточия и без табов).
"""
...
# подсказки для «один пробел» разделителя
KEY_HINTS = (
    "модель","совместимые модели","совместимость","ресурс","цвет печати","тип печати",
    "гарантия","вес","размер","формат","бренд","производитель","артикул","принтеры"
)
KEY_HINTS_LC = {k.lower() for k in KEY_HINTS}
...
def kv_li(line: str) -> str:
    ...
    # 1) уже разметили strong — просто завернуть в <li>
    ...
    # 3) «Ключ  (2+ пробелов/табы)  Значение»
    ...
    # 4) НОВОЕ: «Ключ (ОДИН пробел) Значение»
    m3 = re.match(r"^\s*([^:<>{}\n]{1,120}?)\s+(.+?)\s*$", s, flags=re.S)
    if m3:
        key_raw = m3.group(1).strip()
        val_raw = m3.group(2).strip()
        key = _compact_spaces(key_raw)
        val = _compact_spaces(val_raw)
        # эвристики, чтобы не ловить обычные предложения
        looks_like_value = bool(re.search(r"\d", val) or re.search(r"[A-ZА-Я]{2,}", val) or len(val) >= 3)
        key_ok = (key.lower() in KEY_HINTS_LC) or (len(key) <= 32 and not re.search(r"\s..\s", key))
        if key_ok and looks_like_value:
            return f"<li><strong>{esc(key)}:</strong> {esc(val)}</li>"
...
def transform_heading_then_p_kv(html_txt: str) -> str:
    """
    <p>Характеристики</p><p>строка</p>... → «Характеристики:» + <ul><li>…</li></ul>
    Внутри <p> поддержаны: «Ключ: Значение», «Ключ  (много пробелов/табы)  Значение»
    и (НОВОЕ) «Ключ (один пробел) Значение» с эвристиками.
    """
    def repl(m: re.Match) -> str:
        ...
        for ptxt in re.findall(r"<p[^>]*>(.*?)</p>", block_ps, flags=re.S|re.I):
            t = re.sub(r"<[^>]+>", " ", ptxt)  # не схлопываем одинарные пробелы!
            t = t.strip()
            if t: items.append(t)
        if not items:
            return m.group(0)
...
def transform_characteristics_paragraphs(html_txt: str) -> str:
    ...
    # дополнительно: кейс, когда всё внутри одного <p> списком — оставляем прежнюю логику
...
def beautify_existing_html(html_txt: str) -> str:
    step0 = transform_heading_then_p_kv(html_txt)
...
def build_seo_block(name_text: str, vendor_text: str, tail_plain: str) -> str:
    ...
    if not grabbed:
        ...
        compat = re.search(r"(?i)\b(HP|Canon|Epson|Brother|Kyocera|Samsung|Ricoh|Xerox|Sharp|Lexmark|OKI|Panasonic|Konica Minolta|Pantum)\b",
                           (name_text or "") + " " + (tail_plain or ""))
        bits = []
        ...
        grabbed = " ".join(bits)
    body = esc(grabbed)
    return ('<div style="font-family: \'Times New Roman\', Times, serif; font-size:15px; line-height:1.55;">'
            f'<p>{body}</p>'
            '</div>')
...
# ---------- Постфикс: разворачиваем &lt;strong&gt; внутри <li> ----------
